#!/usr/bin/env python3
"""Run recording-level evaluation for completed RawCNN baseline runs."""
...
from eval_raw_cnn_recording import evaluate_recordings, write_per_recording  # noqa: E402
...
def infer_test_index(project_root: Path, experiment: str) -> Path | None:
    ...
    if experiment.startswith("raw_cnn_ottawa_speed_"):
        ...
        return data_root / "ottawa_leave_speed" / f"target_speed_{target}" / "test_windows.csv"
    if experiment.startswith("raw_cnn_hust_bearing_type_"):
        target = experiment.removeprefix("raw_cnn_hust_bearing_type_")
        return data_root / "hust_leave_bearing_type" / f"target_bearing_type_{target}" / "test_windows.csv"
    if experiment.startswith("raw_cnn_paderborn_condition_"):
        target = experiment.removeprefix("raw_cnn_paderborn_condition_")
        return data_root / "paderborn_leave_condition" / f"target_{target}" / "test_windows.csv"
    return None
...
def main() -> None:
    ...
    completed = []
    skipped = []
    for checkpoint_path in sorted(checkpoint_root.glob("*/best.pt")):
